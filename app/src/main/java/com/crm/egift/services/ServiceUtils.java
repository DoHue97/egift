...
import com.crm.egift.BuildConfig;
import com.crm.egift.activity.LoginActivity;
import com.crm.egift.model.ResultCode;
...
public class ServiceUtils {
        ...
        try {
            ...
            if (conn.getResponseCode() == 200) {
                ...
                if (refreshResult.getString("responseCode").equals("200")) {
                    ...
                    String exp = refreshResult.getString("expiration_date");
    ...
    private static void setHttpPost(HttpURLConnection conn, Boolean isApiKey, Boolean isToken, Boolean isRefresh) throws ProtocolException, TimeoutException {
        ...
        if (isApiKey) {
            Log.d(TAG, "Using api_key: " + BuildConfig.api_key); //Constants.CrmConfig.API_KEY
            conn.setRequestProperty("api_key", BuildConfig.api_key);
        } else if (isToken) {
            if (isRefresh) {
    ...
    private static void setHttpGet(HttpURLConnection conn, Boolean useApiKey, Boolean isToken, Boolean isRefresh) throws ProtocolException, TimeoutException {
        ...
        conn.setRequestProperty("Content-Type", "application/json");
        if (useApiKey) {
            Log.d(TAG, "Using api_key: " + BuildConfig.api_key);
            conn.setRequestProperty("api_key", BuildConfig.api_key);
        } else if (isToken) {
            if (isRefresh) {
                Log.d(TAG, "Using REFRESH_TOKEN: " + Constants.CrmConfig.REFRESH_TOKEN);
...
//        conn.setDoOutput(true);
    ...
    public static JSONObject sendPutRequestWithToken(String urlStr, JSONObject requestBody, Context context) throws Exception {
        return sendPutRequest(urlStr, requestBody, context, false, true, false);
    }

    public static JSONObject sendPutRequestWithApiKey(String urlStr, JSONObject requestBody, Context context) throws Exception {
        return sendPutRequest(urlStr, requestBody, context, true, false, false);

    }

    private static void setHttpPut(HttpURLConnection conn, Boolean isApiKey, Boolean isToken, Boolean isRefresh) throws ProtocolException, TimeoutException {
        conn.setReadTimeout(10000);
        conn.setConnectTimeout(5000);
        conn.setUseCaches(false);
        conn.setRequestProperty("Content-Type", "application/json");
    ...
    public static JSONObject sendPutRequest(String urlStr, JSONObject requestBody, Context context, Boolean isApiKey, Boolean isToken, Boolean isRefresh) throws Exception {
        JSONObject jsonResult = null;
        ...
        try {
            ...
            Log.d(TAG, "URL: " + url.toString());
            conn = (HttpURLConnection) url.openConnection();
            setHttpPut(conn, isApiKey, isToken, isRefresh);
            OutputStream os = conn.getOutputStream();
            BufferedWriter writer = new BufferedWriter(
                    new OutputStreamWriter(os, StandardCharsets.UTF_8));
            writer.write(requestBody.toString());
    ...
    public static void validateForceLogout(String code, Context context, Boolean isLogOut) {
        ...
        try {
            if (isLogOut && code.equals("401")) {
                ...
                // Todo: show message ...
                Log.e("EXPIREDDDD", "Session expired. Please log in again.");
